...
const ExperienceCard: React.FC<ExperienceCardProp> = ({
  ...
  useEffect(() => {
    ...
    const observer = new MutationObserver(() => {
      ...
      const shouldBreak =
        titleWidthInMn + subtitleWidthInMn + timelineWidthInMn + pagePxMn * 2 >
        205;
      ...
      <div>
        {title && (
          <span ref={titleRef}>
            <EditableText
              text={title}
              path={`experience.${sectionIndex}.items[${itemIndex}].title`}
        ...
        <span>
          {subtitle && (
            <>
              {!lineBreak && (
                <span className="font-normal text-gray-400 theme-divider-color mx-1 align-middle">
                  丨
                </span>
              )}
          ...
          {timeline && (
            <span
              ref={timelineRef}
              className={`text-sm mt-1 theme-text-color float-right text-details ${
                lineBreak ? "-mt-5" : ""
              }`}
            >
              <EditableText
                text={timeline}
                path={`experience.${sectionIndex}.items[${itemIndex}].timeline`}
              />
            </span>
          )}
        </span>
      ...
      <p className="mb-1">
        {tech && (
          <EditableText
            type={"tech"}
            text={tech}
            path={`experience.${sectionIndex}.items[${itemIndex}].tech`}
          />
        )}
      </p>
      <ul className="list-disc list-inside mb-2">
        {details
          ...
          .map((detail, detailIndex) => (
            <li
              key={detailIndex}
              className="theme-marker-color font-normal text-details"
            >
              <EditableText
                text={detail}
                path={`experience.${sectionIndex}.items[${itemIndex}].details[${detailIndex}]`}
              />
            </li>
          ))}
      </ul>
    ...
    <div>
      {data?.map((part, sectionIndex) => (
        <div key={sectionIndex}>
          <div className="flex items-center">
            {part.icon && (
              <EditableText
                type={"icon"}
                text={part.icon}
                path={`experience.${sectionIndex}.icon`}
            ...
            {part.section && (
              <EditableText
                text={part.section}
                path={`experience.${sectionIndex}.section`}
          ...
          {/* 分割线 */}
          {part.section && (
            <div className="border-solid border-t-2 theme-divider-color mb-1"></div>
          )}
